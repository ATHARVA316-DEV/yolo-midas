...
Includes simple collision avoidance logic:
- Displays "STOP!" if a "NEAR" object is in the center of the screen.
"""
        ...
        while True:
            ...
            for r in yolo_results:
                for box in r.boxes:
                    ...
                    # --- NEW 7b: Simple Collision Avoidance Logic ---
                    
                    # Check if the object is directly in front of us
                    # 1. Is it "NEAR"?
                    # 2. Is the center of the screen (w0 / 2) between the
                    #    object's left (x1) and right (x2) edges?
                    is_in_front = (x1 < (w0 / 2) < x2)
                    
                    if distance_label == "NEAR" and is_in_front:
                        # Display a large "STOP" warning on the frame
                        cv2.putText(frame, "STOP!", (w0 // 2 - 150, h0 // 2),
                                    cv2.FONT_HERSHEY_TRIPLEX, 3.0, (0, 0, 255), 5)
                        
                        # Also make the bounding box for this object RED
                        color = (0, 0, 255) # Override color to Red
